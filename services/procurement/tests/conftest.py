...
def published(monkeypatch: pytest.MonkeyPatch) -> Published:
    """Події, які сервіс опублікував би в шину."""
    sent: Published = []
...
async def client(session: AsyncSession) -> AsyncIterator[AsyncClient]:
    ...
    async with AsyncClient(
        ...
        base_url="http://test",
        headers=as_user("procurement.read,procurement.write,catalog.read"),
    ) as c:
        yield c
...
def low(part: str, free: str, min_qty: str) -> dict[str, str]:
    """stock.low так, як його шле inventory."""
    return {
        "part_id": part,
        "sku": CATALOG[part]["sku"],
        "brand": CATALOG[part]["brand"],
        "name": CATALOG[part]["name"],
        "unit": CATALOG[part]["unit"],
        "free": free,
        "min_qty": min_qty,
    }
