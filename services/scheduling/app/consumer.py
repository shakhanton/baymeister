...
Дошка на день показує імʼя, телефон і держномер без запитів до сусідів: це
...
async def on_customer_updated(event: dict[str, Any]) -> None:
    ...
    logger.info("customer.updated %s → оновлено записів: %d", payload["id"], changed)
...
async def on_vehicle_updated(event: dict[str, Any]) -> None:
    ...
    async with SessionLocal() as session:
        ...
        await session.commit()
    logger.info("vehicle.updated %s → оновлено записів: %d", payload["id"], changed)
